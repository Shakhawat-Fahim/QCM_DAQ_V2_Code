...
class SharpMemoryDisplay(framebuf.FrameBuffer):
    ...
    def __init__(self, spi, cs, width=128, height=128, sd_cs_pin=None,
        ...
        Args:
            ...
            baudrate   : SPI clock for LCD transactions (default 2 MHz, the
                         Sharp LS013B4DN04 datasheet max). The LCD class sets
                         SPI to this speed before every show(). If only using
                         LCD, you can simply init SPI at this speed.
        """
        self.spi       = spi
        self.cs        = cs
    ...
    def show(self):
        ...
        self._claim_bus()
        # Set SPI to LCD's preferred speed (Sharp LCDs max out at ~2 MHz)
...
class SDCardStorage:
    ...
    def __init__(self, spi, cs, mount_point="/sd", lcd_cs_pin=None,
        ...
        Args:
            ...
            baudrate    : target SPI speed for SD data operations after mount
                          (default 10 MHz). Modern cards handle 25-50 MHz;
                          lower values are safer for long wires / breadboards.
                          Set to 1_000_000 for maximum compatibility.
        """
        self.spi         = spi
        self.cs          = cs
    ...
    def _claim_bus(self):
        """Deselect LCD and set SPI to SD's preferred speed.
           Called before every SD transaction."""
        if self.lcd_cs_pin is not None:
            self.lcd_cs_pin.value(0)
        # Only change speed after mount; sdcard.py manages speed during init
        if self._mounted:
            self.spi.init(baudrate=self.baudrate, polarity=0, phase=0)
    ...
    def mount(self, retries=3):
        """
        Initialize the SD card and mount it as a FAT filesystem.
        Requires sdcard.py driver in /lib.

        Bus-preparation sequence (critical when sharing SPI with the LCD):
          1. Force LCD CS to idle LOW so the LCD ignores the bus
          2. Reinitialize SPI at 400 kHz (SD spec requires 100-400 kHz for init)
          3. Hold SD CS HIGH, pulse clock >200 cycles to flush any stuck state
          4. Call sdcard.SDCard() — this performs CMD0/CMD8/etc. handshake
          5. Retry on timeout (common failure mode after LCD activity)

        After a successful mount, the SD driver sets SPI back to a higher
        speed for data transfers.
        """
        if self._mounted:
            return True
        ...
        for attempt in range(retries):
            try:
                ...
                # Step 2 — Force SPI back to slow init speed.
                # The LCD runs at 2 MHz and leaves the bus there; SD init
                # MUST happen at <=400 kHz or the card won't respond.
                self.spi.init(baudrate=400_000, polarity=0, phase=0)

                # Step 3 — Deselect the card and pulse many clock cycles to
                # flush any stuck/partial state. We send 32 bytes of 0xFF
                # with CS HIGH, which is 256 clock pulses — well beyond the
                # 74 the SD spec requires for cold-start.
                self.cs.value(1)
                time.sleep_ms(20)
                self.spi.write(b"\xff" * 32)
                time.sleep_ms(20)

                # Step 4 — Hand off to the low-level driver
                ...
                try:
                    ...
                    pass
                # Pause a bit longer between retries
                time.sleep_ms(100 * (attempt + 1))

        # All retries failed — raise the last error
        raise last_err if last_err else OSError("SD mount failed")
